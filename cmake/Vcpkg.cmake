# Basically a copy of AutoVcpkg.cmake with a few minor changes
...
function (vcpkg_bootstrap)
  ...
  if (NOT AUTO_VCPKG_EXECUTABLE)
    ## write bootstrap.cmake
    set(vcpkg_bootstrap_content [===[
    find_program(VCPKG_EXECUTABLE vcpkg PATHS "${CMAKE_CURRENT_LIST_DIR}")
    if (NOT VCPKG_EXECUTABLE)
      if (WIN32)
        execute_process(COMMAND "${CMAKE_CURRENT_LIST_DIR}/bootstrap-vcpkg.bat" -disableMetrics WORKING_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}")
      else ()
        execute_process(COMMAND "${CMAKE_CURRENT_LIST_DIR}/bootstrap-vcpkg.sh" -disableMetrics WORKING_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}")
      endif()
    endif()
    ]===])
    file(WRITE "${AUTO_VCPKG_ROOT}/VcpkgBootstrap.cmake" "${vcpkg_bootstrap_content}")
    execute_process(COMMAND ${CMAKE_COMMAND} -P "${AUTO_VCPKG_ROOT}/VcpkgBootstrap.cmake" WORKING_DIRECTORY ${AUTO_VCPKG_ROOT})
  endif()
endfunction()
...
function (vcpkg_install)
    ...
    message(STATUS "AutoVcpkg: vcpkg_install() called to install: ${packages}")
