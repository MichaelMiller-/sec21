...
namespace sec21::event::backend
{
   class sdl2
   {
    public:
      ...
      {
         return sec21::match(
            event, [](auto const& value) -> std::optional<SDL_Event> { return transform_event(value); },
    ...
    private:
      event_t to_event(SDL_Event const& event) const noexcept
      {
         ...
         case SDL_QUIT:
            return close_window{};
         case SDL_KEYDOWN:
            return make_event<pressed<key>>((event.key.keysym.mod & KMOD_ALT), (event.key.keysym.mod & KMOD_CTRL),
                                            (event.key.keysym.mod & KMOD_GUI), (event.key.keysym.mod & KMOD_SHIFT),
                                            to_keycode(event.key.keysym.sym));
         case SDL_KEYUP:
            return make_event<released<key>>((event.key.keysym.mod & KMOD_ALT), (event.key.keysym.mod & KMOD_CTRL),
                                             (event.key.keysym.mod & KMOD_GUI), (event.key.keysym.mod & KMOD_SHIFT),
                                             to_keycode(event.key.keysym.sym));
         ...
         case SDL_MOUSEWHEEL:
            return mouse_wheel{event.wheel.x, event.wheel.y};
         default:
      ...
      static SDL_Event transform_event(mouse_wheel const& value) noexcept
      {
         // clang-format off
         return SDL_Event{ 
      ...
      static SDL_Event transform_event(close_window const&) noexcept { return SDL_Event{.type = SDL_QUIT}; }
   };
