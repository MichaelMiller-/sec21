...
namespace sec21::units
{
...
#endif
      constexpr quantity(Q const& q) noexcept : m_value{quantity_cast<quantity>(q).value()}
      {
      }

      [[nodiscard]] constexpr auto value() const noexcept { return m_value; }

      [[nodiscard]] constexpr auto operator-() const noexcept { return quantity(-value()); }

      [[nodiscard]] static constexpr quantity min() noexcept { return std::numeric_limits<value_t>::lowest(); }
      [[nodiscard]] static constexpr quantity max() noexcept { return std::numeric_limits<value_t>::max(); }
   ...
   template <Quantity Q1, Quantity Q2>
      requires SameDimension<Q1, Q2>
#else
   template <typename Q1, typename Q2, std::enable_if_t<has_same_dimension<Q1, Q2>::value, bool> = true>
#endif
   ...
   template <typename Q1, typename Q2,
             std::enable_if_t<is_quantity<Q1>::value && is_quantity<Q2>::value && has_same_dimension<Q1, Q2>::value,
                              bool> = true>
#endif
   ...
   template <
      ...
      std::enable_if_t<is_quantity<Q1>::value && is_quantity<Q2>::value && has_same_dimension<Q1, Q2>::value == false,
                       bool> = true>
#endif
   constexpr decltype(auto) operator*(Q1 const& lhs, Q2 const& rhs)
   {
...
#else
   template <typename Q1, typename S, std::enable_if_t<is_quantity<Q1>::value && std::is_scalar<S>::value, bool> = true>
#endif
...
#else
   template <typename S, typename Q1, std::enable_if_t<is_quantity<Q1>::value && std::is_scalar<S>::value, bool> = true>
#endif
...
#else
   template <typename Q, typename S, std::enable_if_t<is_quantity<Q>::value && std::is_scalar<S>::value, bool> = true>
#endif
...
namespace sec21::units
{
   namespace detail
   {
      ...
      auto from_string(std::string_view input) -> Q
      {
...
#endif
         const auto valid_abbreviations =
            std::apply([&](auto... n) { return std::array{to_abbreviation(n)...}; }, valid_types);
         ...
         if (it == end(valid_abbreviations))
            throw std::runtime_error("Couldn't match input quantity with valid dimension type. Invalid dimension: " +
                                     unit);
   ...
   {
      j = nlohmann::json{{"quantity", detail::to_string(obj)}};
   }
