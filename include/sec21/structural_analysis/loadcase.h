...
namespace sec21::structural_analysis
{
   ...
   enum class LoadDuration { Long, Medium, Short, Instantaneous };

   enum class CoordinateSystem { Global, Local };

   enum class Direction { X = 0, Y = 1, Z = 2 };

   enum class ActionType { Permanent, Variable, Accidental };
...
namespace sec21::structural_analysis
{
   NLOHMANN_JSON_SERIALIZE_ENUM(ActionType, {
                                               {ActionType::Permanent, "Permanent"},
                                               {ActionType::Variable, "Variable"},
                                               {ActionType::Accidental, "Accidental"},
                                            })

   NLOHMANN_JSON_SERIALIZE_ENUM(LoadType, {
                                             {LoadType::SelfWeight, "SelfWeight"},
                                             {LoadType::Others, "Others"},
                                             {LoadType::Prestress, "Prestress"},
                                             {LoadType::Dynamic, "Dynamic"},
                                             {LoadType::Static, "Static"},
                                             {LoadType::Temperature, "Temperature"},
                                             {LoadType::Wind, "Wind"},
                                             {LoadType::Snow, "Snow"},
                                             {LoadType::Maintenance, "Maintenance"},
                                             {LoadType::Fire, "Fire"},
                                             {LoadType::Moving, "Moving"},
                                             {LoadType::Dynamic, "Dynamic"},
                                             {LoadType::Seismic, "Seismic"},
   ...
   NLOHMANN_JSON_SERIALIZE_ENUM(LoadDuration, {
                                                 ...
                                                 {LoadDuration::Instantaneous, "Instantaneous"},
                                              })
   ...
   {
      j = nlohmann::json{{"member_id", obj.member_id}, {"delta_t", obj.delta_t}, {"alpha_t", obj.alpha_t}};
   }
   ...
   {
      j =
         nlohmann::json{{"name", obj.name}, {"node_load", obj.node_load}, {"temperature_loads", obj.temperature_loads}};
   }
