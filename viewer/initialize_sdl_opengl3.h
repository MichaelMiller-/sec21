...
struct sdl_event_impl
{
   ...
   explicit sdl_event_impl(SDL_Event const& event) noexcept : data{event} {}
};
   ...
   {
      {
         ...
         SDL_VERSION(&compiled);
         spdlog::debug("compiled SDL version {}.{}.{}", compiled.major, compiled.minor, compiled.patch);
      }

      SDL_version linked;
      SDL_GetVersion(&linked);
      spdlog::info("initialize SDL version {}.{}.{}", linked.major, linked.minor, linked.patch);
      {
         const auto num_drives = SDL_GetNumVideoDrivers();
         spdlog::info("video drivers: {}", num_drives);
         for (auto i = 0; i < num_drives; ++i)
            spdlog::info(" [{}]:{}", i, SDL_GetVideoDriver(i));
      }

      // TODO: only works with sdl > 2.0.14
      if (SDL_VideoInit(settings.headless ? "offscreen" : nullptr) < 0)
      ...
      spdlog::info("OpenGL version {}.{}", 3, 1);
      // Use OpenGL 3.1 core
      ...
      if (window =
             SDL_CreateWindow("sec21::viewer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, settings.screen_width,
                              settings.screen_height, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_ALLOW_HIGHDPI);
          window == nullptr) {
